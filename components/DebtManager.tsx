...
interface DebtManagerProps {
  ...
  onOpenUnifiedDebtForm?: (mode: 'to_me' | 'on_me' | 'repayment' | 'edit', debtId?: string, prefillPerson?: string, debt?: Debt) => void;
}
...
export const DebtManager: React.FC<DebtManagerProps> = ({ 
  ...
  const openAdd = (prefilledPerson?: string, specificType?: 'to_me' | 'on_me') => {
    if (onOpenUnifiedDebtForm) {
      const mode = specificType || (statusFilter === 'on_me' ? 'on_me' : 'to_me');
      onOpenUnifiedDebtForm(mode, undefined, prefilledPerson);
    }
  };

  const openEdit = (d: Debt) => {
    if (onOpenUnifiedDebtForm) {
      onOpenUnifiedDebtForm('edit', d.id, d.personName, d);
    }
  };

  const openPaymentModal = (debt: Debt, prefillAmount?: number, _installmentId?: string) => {
    if (onOpenUnifiedDebtForm) {
      onOpenUnifiedDebtForm('repayment', debt.id);
    }
  };
