...
// Advanced robust PII and token regex patterns matching user requirements exactly
...
/**
 * Masks phone numbers to retain only the last 4 digits and mask preceding digits with stars.
 */
export function maskPhone(number: string): string {
  if (!number || typeof number !== 'string') return REDACTED_PHONE;
  try {
